@extends('components/layouts.layout')
@section('content')
    @include('components/presentational/header',['path'=>''])
    <main>
        <section class="konsultasi__page">
            <div class="container">
                <div class="row">
<<<<<<< HEAD
                    {{-- <iframe src="https://livechat.medkomtek.net?kdUser={{ session()->get('tokenUser') }}" frameborder="0" width="100%" height="600" ></iframe> --}}
                    {{--<iframe src="https://livechat.klikdokter.com/?filter=124,38,9&kdUser={{ session()->get('tokenUser') }}" frameborder="0" width="100%" height="600" ></iframe>--}}
			<iframe src="https://livechat.medkomtek.net?kdUser={{ session()->get('tokenUser') }}" frameborder="0" width="100%" height="600" ></iframe>
=======
                    <iframe src="https://livechat.klikdokter.com/?filter=124,38,9&kdUser={{ session()->get('tokenUser') }}" frameborder="0" width="100%" height="600" ></iframe>
>>>>>>> 2b71523f13b40881e5cc241138ff3f03386ef458
                </div>
            </div>
        </section>
    </main>
@endsection
